...
The database is in \code{GSEABase::GeneSetCollection} format and
includes 30 pathways, each of which is included in one of 3 different
collections. Following Broad conventions, each collection is
identified by a "bcCategory" and "bcSubCategory" fields, which
`gep2pep` puts together into a single collection identifier using
`makeCollectionIDs`.


```{r}
colltypes <- sapply(db, collectionType)
cats <- sapply(colltypes, bcCategory)
subcats <- sapply(colltypes, bcSubCategory)
print(cats)
print(subcats)
...
repository and loads the description of the pathway "E47_01" from the
collection "c3_TFT":

```{r}
rp
description(rp$get("c3_TFT_sets")[["E47_01"]])
```

\code{rp$get} is a \code{repo} command, see later in this vignette.
